...
This is a document on how we have compiled our experimental data on infections of lab mice with the parasite *Eimeria*
...
### Prepare vectors of names, for selecting relevant columns to one specific category
```{r}
#select columns: 
...
In each row there is a link to the raw data for each experiment

```{r pressure, echo=FALSE}
OV <- read.csv("https://raw.githubusercontent.com/derele/Eimeria_Lab/master/Eimeria_Lab_overview.csv")
```

### Selecting the challenge experiments from our data
Creating a list of the names of each experiments.
Use it later, to select the experiments from ov.
```{r}
...
we select the challenge experiments and the weight columns 
...
### Binding all the data frames, one after the other
reduce: works on 1st and 2nd element, produces result and then uses the result with the 3rd element and so on
...
### there are more in in the weights table which are not found in the oocysts though
```{r}
...
wo samples with no result records
**[1] "LM0205" "LM0290"**
```{r}
