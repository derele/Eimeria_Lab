...
ExpeDF_003$Mouse_strain <- factor(ExpeDF_003$Mouse_strain,
                                  ...
                                  labels = c("M.m.domesticus \n(STRA)", 
                                             ...
                                             "M.m.musculus \n(PWD)"))
# Add OPG status
ExpeDF_003$OPGstatus[is.na(ExpeDF_003$OPG)] = "na"
ExpeDF_003$OPGstatus[!is.na(ExpeDF_003$OPG) & ExpeDF_003$OPG > 0] = "positive"
ExpeDF_003$OPGstatus[!is.na(ExpeDF_003$OPG) & ExpeDF_003$OPG == 0] = "negative"
ExpeDF_003$OPGstatus = as.factor(ExpeDF_003$OPGstatus)

# Enter Eimeria species
ExpeDF_003$Eimeria_species[ExpeDF_003$infection_isolate %in% c("E88", "Eflab", "EfLab")] = "E.falciformis"
ExpeDF_003$Eimeria_species[ExpeDF_003$infection_isolate %in% c("E64", "EI64", "E139")] = "E.ferrisi"

# Keep only mice that survived 11 days
survivors <- names(table(ExpeDF_003$EH_ID))[table(ExpeDF_003$EH_ID) %in% 12]
ExpeDF_003 <- ExpeDF_003[ExpeDF_003$EH_ID %in% survivors,]

# calculate relative weight loss compared to dpi0
ExpeDF_003 <- calculateWeightLoss(ExpeDF_003, startingDay = 0)

# calculate summary statistics on weight loss
summaryWeight_003 <- summarySE(ExpeDF_003, measurevar = "relativeWeight",
                           groupvars=c("Mouse_strain", "Mouse_subspecies", "Eimeria_species",
                                       "infection_isolate", "dpi"))
summaryWeight_003$ci[is.na(summaryWeight_003$ci)] <- 0

# calculate summary statistics on oocysts shedding
summaryOocysts_003 <-summarySE(ExpeDF_003, measurevar="OPG", 
                           groupvars=c("Mouse_strain", "Mouse_subspecies", "Eimeria_species",
                                       "infection_isolate", "dpi"))
summaryOocysts_003$ci[is.na(summaryOocysts_003$ci)] <- 0
```

## 1. Weight loss
```{r weightalong3, echo=FALSE, warning=FALSE, fig.width = 12, fig.height = 10}
ggplot(ExpeDF_003, aes(x = dpi, y = weight, fill = OPGstatus)) +
  geom_rect(aes(xmin = 3, xmax = 8, ymin = -Inf, ymax = Inf), fill = "lightgrey", alpha = .5) +
  geom_line(aes(group = EH_ID, col = Mouse_strain), size = 2, alpha = 0.5) +
  geom_point(size=4, pch = 21, color = "black")+
  scale_fill_manual(values = c("lightgrey", "yellow")) +
...
ggplot(ExpeDF_003, aes(x = dpi, y = relativeWeight, fill = OPGstatus)) +
  geom_rect(aes(xmin = 3, xmax = 8, ymin = 80, ymax = 110), fill = "lightgrey", alpha = .5) +
  geom_line(aes(group = EH_ID, col = Mouse_strain), size = 2, alpha = 0.5) +
  geom_point(size=4, pch = 21, color = "black")+
  scale_fill_manual(values = c("lightgrey", "yellow")) +
...
ggplot(summaryWeight_003, aes(x = dpi, y = relativeWeight))+
  # geom_rect(aes(xmin = 3, xmax = 8, ymin = 85, ymax = 115), fill = "lightgrey", alpha = .5) +
  geom_errorbar(aes(ymin = relativeWeight - ci, ymax = relativeWeight + ci)) +
  geom_line(aes(group = Mouse_strain, col = Mouse_strain), size = 2, alpha = 0.5) +
  geom_point(aes(fill = Mouse_strain), size=4, pch = 21, color = "black") +
...
# ExpeDF_001Symptomatic <- ExpeDF_001[ExpeDF_001$dpi %in% 3:8,]
# max.loss_003 <- do.call("rbind", by(ExpeDF_001Symptomatic, ExpeDF_001Symptomatic$EH_ID, function (x){
#   m.loss <- which(x$relativeWeight == min(x$relativeWeight, na.rm=TRUE))
#   x[m.loss,]
# }))
...
ggplot(ExpeDF_003, aes(x = dpi, y = OPG, fill = infection_isolate))+
  # geom_rect(aes(xmin = 3, xmax = 8, ymin = -Inf, ymax = Inf), fill = "lightgrey", alpha = .5) +
  geom_line(aes(group = EH_ID, col = Mouse_strain), alpha = 0.5, size = 2) +
  geom_point(size=3, aes(color = Mouse_strain))+
  # scale_color_manual(values = c("blue", "purple", "red")) +
...
ggplot(summaryOocysts_003, aes(x = dpi, y = OPG))+
  # geom_rect(aes(xmin = 3, xmax = 8, ymin = -Inf, ymax = Inf), fill = "lightgrey", alpha = .5) +
  geom_errorbar(aes(ymin = OPG - ci, ymax = OPG + ci)) +
  geom_line(aes(group = Mouse_strain, col = Mouse_strain), size = 2, alpha = 0.5) +
  geom_point(aes(fill = Mouse_strain), size=4, pch = 21, color = "black") +
...
# Cumulative OPG along experiment
all.sum.oocysts_003 <- do.call("rbind", by(ExpeDF_003, ExpeDF_003$EH_ID, function (x){
  x$sum.oo <- sum(x$OPG, na.rm = TRUE)
  x
}))
sum.oocysts_003 <- all.sum.oocysts_003[!duplicated(all.sum.oocysts_003$EH_ID),]

ggplot(sum.oocysts_003, aes(Mouse_strain, sum.oo, color=infection_isolate)) +
  geom_boxplot(color = "black")+
  ggtitle("Sum of oocysts shed along the experiment") +
  facet_wrap(~infection_isolate) +
  geom_jitter(width=0.1, size=7, pch = 21,
...
summary(lm(sum.oo~Mouse_strain, data = sum.oocysts_003))
```
...
# calculate relative weight loss compared to dpi0 for E88
ExpeDF_005_E88 <- calculateWeightLoss(ExpeDF_005_E88, startingDay = 6)
...
summaryWeight_005_E64 <- summarySE(ExpeDF_005_E64, measurevar = "relativeWeight",
                           groupvars=c("Mouse_subspecies", "Eimeria_species",
                                       "dpi"))
...
ggplot(summaryWeight_005_E64, aes(x = dpi, y = relativeWeight))+
  geom_rect(aes(xmin = 3, xmax = 8, ymin = 85, ymax = 115), fill = "lightgrey", alpha = .5) +
  geom_errorbar(aes(ymin = relativeWeight - ci, ymax = relativeWeight + ci)) +
  geom_line(aes(group = Mouse_subspecies, col = Mouse_subspecies), size = 2, alpha = 0.5) +
  geom_point(aes(fill = Mouse_subspecies), size=4, pch = 21, color = "black") +
  # scale_color_manual(values = c("blue", "purple", "red")) +
  # scale_fill_manual(values = c("blue", "purple", "red")) +
  mytheme +
  facet_grid(Mouse_subspecies ~ .) +
...
ggplot(max.loss_005_E64, aes(Mouse_subspecies,100-relativeWeight, color=Mouse_subspecies)) +
  geom_boxplot(color = "black")+
  ggtitle("Maximum weigth loss during the symptomatic phase") +
  geom_jitter(width=0.1, size=7, pch = 21,
              color = "black", aes(fill = HybridStatus), alpha = 0.8) +
  labs(y= "Maximum weigth loss relative to weight at dpi 3", x= "Mouse subspecies")+
  mytheme +
  theme(legend.position="none")
# summary(glmer(relativeWeight ~ HybridStatus + 1|Mouse_subspecies, data = max.loss_005_E64))
```
...
ggplot(ExpeDF_005_E88, aes(x = dpi, y = weight, fill = OPGstatus)) +
  geom_rect(aes(xmin = 6, xmax = 11, ymin = -Inf, ymax = Inf), fill = "lightgrey", alpha = .5) +
  geom_line(aes(group = EH_ID, col = Mouse_strain), size = 2, alpha = 0.5) +
  # scale_color_manual(values = c("blue", "purple", "red")) +
  geom_point(size=4, pch = 21, color = "black")+
  scale_fill_manual(values = c("lightgrey", "yellow")) +
  ...
  theme(strip.text.y = element_text(size = 15)) +
  theme(legend.position="none") +
  ggtitle("Weight along experiment compared to dpi3 (E88)", subtitle = "yellow : oocysts detected in feces")

ggplot(ExpeDF_005_E88, aes(x = dpi, y = relativeWeight, fill = OPGstatus)) +
  geom_rect(aes(xmin = 6, xmax = 11, ymin = 80, ymax = 110), fill = "lightgrey", alpha = .5) +
  geom_line(aes(group = EH_ID, col = Mouse_strain), size = 2, alpha = 0.5) +
  # scale_color_manual(values = c("blue", "purple", "red")) +
  geom_point(size=4, pch = 21, color = "black")+
  scale_fill_manual(values = c("lightgrey", "yellow")) +
  ...
  theme(strip.text.y = element_text(size = 15)) +
  theme(legend.position="none") +
  ggtitle("Relative weight along experiment compared to dpi3 (E88)", subtitle = "yellow : oocysts detected in feces")
```
...
ggplot(summaryWeight_005_E88, aes(x = dpi, y = relativeWeight))+
  geom_rect(aes(xmin = 6, xmax = 11, ymin = 85, ymax = 115), fill = "lightgrey", alpha = .5) +
  geom_errorbar(aes(ymin = relativeWeight - ci, ymax = relativeWeight + ci)) +
  geom_line(aes(group = Mouse_subspecies, col = Mouse_subspecies), size = 2, alpha = 0.5) +
  geom_point(aes(fill = Mouse_subspecies), size=4, pch = 21, color = "black") +
  # scale_color_manual(values = c("blue", "purple", "red")) +
...
ggplot(max.loss_005_E88, aes(HybridStatus,100-relativeWeight, color=infection_isolate)) +
  ...
  geom_jitter(width=0.1, size=7, pch = 21,
              color = "black", aes(fill = HybridStatus), alpha = 0.8) +
  labs(y= "Maximum weigth loss relative to weight at dpi 6", x= "Mouse strain")+
  # scale_fill_manual(values = c("blue", "purple", "red")) +
  mytheme +
  theme(legend.position="none")
...
ggplot(max.loss_005_E88, aes(Mouse_subspecies,100-relativeWeight, color=Mouse_subspecies)) +
  geom_boxplot(color = "black")+
  ggtitle("Maximum weigth loss during the symptomatic phase") +
  geom_jitter(width=0.1, size=7, pch = 21,
              color = "black", aes(fill = HybridStatus), alpha = 0.8) +
  labs(y= "Maximum weigth loss relative to weight at dpi 6", x= "Mouse subspecies")+
  mytheme +
  theme(legend.position="none")
