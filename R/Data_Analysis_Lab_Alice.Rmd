...
summary(lm(relativeWeight~Mouse_strain, data = max.loss_003_4))
```
...
summary(lm(sum.oo~Mouse_strain, data = sum.oocysts_003_4))
```
...
summaryWeight_005_E64 <- summarySE(ExpeDF_005_E64, measurevar = "relativeWeight",
                           groupvars=c("HybridStatus", "dpi"), na.rm = T)
...
summary(lm(relativeWeight~HybridStatus, data = max.loss_005_E64))
...
ggplot(ExpeDF_005_E88, aes(x = dpi, y = weight, fill = OPGstatus)) +
  ...
  scale_fill_manual(values = c("lightgrey", "yellow")) +
  mytheme +
  facet_grid(Mouse_subspecies ~ ., scales = "free") +
  scale_x_continuous(breaks = 0:11, name = "Day post infection (dpi)") +
  scale_y_continuous(name = "Weight (g)") +
  theme(strip.text.y = element_text(size = 15)) +
  theme(legend.position="none") +
...
summary(lm(relativeWeight ~ HybridStatus, data = max.loss_005_E88))
